...
const filteredContent = computed(() => {
  let filtered = contentStore.items
...
const saveContent = async () => {
  try {
    if (editingContent.value && contentForm.value.type) {
      await contentStore.updateContent(editingContent.value.id, contentForm.value as Partial<ContentItem>)
    } else if (contentForm.value.type) {
      await contentStore.createContent(contentForm.value as Omit<ContentItem, 'id' | 'createdAt' | 'updatedAt'>)
    }
    closeModal()
  } catch (error) {
...
const toggleStatus = async (content: ContentItem) => {
  try {
    await contentStore.updateContent(content.id, { isActive: !content.isActive })
  } catch (error) {
